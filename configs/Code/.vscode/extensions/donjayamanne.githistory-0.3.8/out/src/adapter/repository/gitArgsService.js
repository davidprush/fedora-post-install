...
let GitArgsService = class GitArgsService {
    ...
    getLogArgs(pageIndex = 0, pageSize = 100, branch = '', searchText = '', relativeFilePath) {
        const allBranches = branch.trim().length === 0;
        const currentBranch = branch.trim() === '*';
        const specificBranch = !allBranches && !currentBranch;
        const logArgs = ['log', '--full-history', constants_1.LOG_FORMAT];
        const fileStatArgs = ['log', '--full-history', `--format=${constants_1.LOG_ENTRY_SEPARATOR}${constants_1.newLineFormatCode}`];
        // tslint:disable-next-line:no-suspicious-comment
        // TODO: Don't we need %n instead of %h
        const counterArgs = ['log', '--full-history', `--format=${constants_1.LOG_ENTRY_SEPARATOR}%h`];
