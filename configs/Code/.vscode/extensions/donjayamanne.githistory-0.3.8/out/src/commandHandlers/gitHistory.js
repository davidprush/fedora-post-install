...
let GitHistoryCommandHandler = class GitHistoryCommandHandler {
    ...
    viewHistory(fileUri) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const gitService = yield this.serviceContainer.get(types_6.IGitServiceFactory).createGitService(workspaceFolder);
            ...
            const fullId = `${startupInfo.port}:${types_6.BranchSelection.Current}:${fileUri ? fileUri.fsPath : ''}`;
            const id = md5(fullId); //Date.now().toString();
            yield this.serviceContainer.get(types_5.IWorkspaceQueryStateStore).initialize(id, workspaceFolder, branchName, types_6.BranchSelection.Current, '', fileUri);
            const queryArgs = [
