...
class ChromeDebugAdapter {
    ...
    onExecutionContextsCleared() {
        ...
        this._scriptsById.forEach(scriptedParseEvent => {
            asyncOperations.push(this.scriptToLoadedSourceEvent('removed', scriptedParseEvent).then(scriptEvent => this._session.sendEvent(scriptEvent)));
        });
        Promise.all(asyncOperations).then(() => this.clearTargetContext());
    }
    ...
    sendLoadedSourceEvent(script) {
        return __awaiter(this, void 0, void 0, function* () {
            const scriptEvent = yield this.scriptToLoadedSourceEvent('new', script);
            ...
            if (targetScriptUrl) {
                ...
                return setBreakpointsPFailOnError.then(body => {
                    if (body.breakpoints.every(bp => !bp.verified)) {
                        return this.unverifiedBpResponseForBreakpoints(args, requestSeq, body.breakpoints, localize(4, null), true);
                    }
                ...
                return Promise.resolve(this.unverifiedBpResponse(args, requestSeq, localize(5, null)));
            }
    ...
    unverifiedBpResponse(args, requestSeq, message, bpsSet = false) {
    ...
    unverifiedBpResponseForBreakpoints(args, requestSeq, breakpoints, defaultMessage, bpsSet = false) {
